		...
		while (stream.read<uint8_t>(attrib)) {
			...
			switch (attrib) {
				...
				case ITEM_ATTR_UPGRADE_CLASSIFICATION: {
					...
					if (!stream.read<uint16_t>(classification)) {
						return ERROR_INVALID_FORMAT;
					}
					break;
				}
...
void Items::parseItemNode(const pugi::xml_node& itemNode, uint16_t id)
{
	...
	for (auto attributeNode : itemNode.children()) {
		...
		if (tmpStrValue == "type") {
			...
			it.wrapable = true;
		}else if (tmpStrValue == "bedtype") {
			it.bedType = asLowerCaseString(valueAttribute.as_string());
		} else if (tmpStrValue == "moveable" || tmpStrValue == "movable") {
			it.moveable = valueAttribute.as_bool();
		} else if (tmpStrValue == "weightreduction") {
			it.weightReduction = pugi::cast<uint16_t>(valueAttribute.value());
		...
		} else if (tmpStrValue == "wearoutassets") {
			it.wearOut = valueAttribute.as_bool();
		} else if (tmpStrValue == "clockexpireassets") {
			it.clockExpire = valueAttribute.as_bool();
		} else if (tmpStrValue == "expireassets") {
			it.expire = valueAttribute.as_bool();
		} else if (tmpStrValue == "expirestopassets") {
