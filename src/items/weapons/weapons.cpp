		...
		case WEAPONACTION_REMOVECOUNT:
			if(g_config.getBoolean(ConfigManager::REMOVE_WEAPON_AMMO)) {
				...
				if (Party* party = player->getParty()) {
					party->updateInformation(player, INFO_SUPPLY, item, 0);
				}
			}
			break;
...
int64_t WeaponMelee::getElementDamage(const Player* player, const Creature*, const Item* item, const uint16_t cleavePercent) const
{
...
int64_t WeaponMelee::getWeaponDamage(const Player* player, const Creature*, const Item* item, bool maxDamage /*= false*/, uint16_t cleavePercent /*= 0*/) const
{
	using namespace std;
	int32_t attackSkill = player->getWeaponSkill(item);
...
int64_t WeaponDistance::getElementDamage(const Player* player, const Creature* target, const Item* item, uint16_t cleavePercent) const
{
	if (elementType == COMBAT_NONE) {
		return 0;
	}
...
int64_t WeaponDistance::getWeaponDamage(const Player* player, const Creature* target, const Item* item, bool maxDamage /*= false*/, uint16_t cleavePercent) const
{
...
int64_t WeaponWand::getWeaponDamage(const Player*, const Creature*, const Item*, bool maxDamage /*= false*/, uint16_t cleavePercent) const
{
	if (maxDamage) {
		return -maxChange;
	}
