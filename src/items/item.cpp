...
Attr_ReadValue Item::readAttr(AttrTypes_t attr, PropStream& propStream)
{
	switch (attr) {
		...
		case ATTR_BOOST: {
			int64_t boost;
			if (!propStream.read<int64_t>(boost)) {
				return ATTR_READ_ERROR;
			}

			setIntAttr(ITEM_ATTRIBUTE_BOOST, boost);
			break;
		}
...
void Item::serializeAttr(PropWriteStream& propWriteStream) const
{
	...
	if (hasAttribute(ITEM_ATTRIBUTE_BOOST)) {
		propWriteStream.write<uint8_t>(ATTR_BOOST);
		propWriteStream.write<int64_t>(getIntAttr(ITEM_ATTRIBUTE_BOOST));
	}
...
std::vector<std::pair<std::string, std::string>>
	...
	if (item) {
		...
		if (it.abilities) {
			...
			if (it.abilities->reflectFlat[0] != 0) {
				ss.str("");
				ss << it.abilities->reflectFlat[0];
...
SoundEffect_t Item::getMovementSound(Cylinder* toCylinder)
{
	...
	case ITEM_TYPE_WEAPON_QUIVER: { return SOUND_EFFECT_TYPE_ITEM_MOVE_QUIVERS; }
								 ...
								 const Item* item /*= nullptr*/, int32_t subType /*= -1*/, bool addArticle /*= true*/)
{
		...
		if (it.weaponType == WEAPON_DISTANCE && it.ammoType != AMMO_NONE) {
			...
			if (it.abilities) {
				...
				if (it.abilities->reflectFlat[0] != 0) {
					...
					s << "damage reflection " << std::showpos << it.abilities->reflectFlat[0] << std::noshowpos;
				}
	...
	if (item && it.classification != 0) {
		s << "\nClassification: " << it.classification;
		s << " Tier: " << std::to_string(item->getBoost());

		if (item->getBoost() > 0) {
			if (it.slotPosition & SLOTP_ARMOR) {
				s << " (Ruse: " << std::to_string(g_forge.getArmorBonus(it.classification, item->getBoost())).substr(0, 4) << ")";
			} else if (it.slotPosition & SLOTP_HEAD) {
				s << " (Momentum: " << std::to_string(g_forge.getHelmetBonus(it.classification, item->getBoost())).substr(0, 4) << ")";
			} else if (it.slotPosition & SLOTP_HAND) {
				s << " (Onslaught: " << std::to_string(g_forge.getWeaponBonus(it.classification, item->getBoost())).substr(0, 4) << ")";
			}
		}
	}
