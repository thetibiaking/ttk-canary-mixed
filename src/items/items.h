...
class ItemType
{
	public:
		...
		uint64_t getGoldValue() const {
			uint64_t value = 0;
			if (id == ITEM_GOLD_COIN) {
				value = 1;
			} else if (id == ITEM_PLATINUM_COIN) {
				value = 100;
			} else if (id == ITEM_CRYSTAL_COIN) {
				value = 10000;
			}
		...
		int64_t defaultBuyValue = 0;
		int64_t defaultSellValue = 0;
