...
BedItem* BedItem::getNextBedItem() const
{
	...
	BedItem* newBed = tile->getBedItem();
	if (newBed) {
		if ((dir == DIRECTION_NORTH && Item::items[newBed->id].bedPartnerDir == DIRECTION_SOUTH) ||
			(dir == DIRECTION_SOUTH && Item::items[newBed->id].bedPartnerDir == DIRECTION_NORTH) ||
			(dir == DIRECTION_WEST && Item::items[newBed->id].bedPartnerDir == DIRECTION_EAST) ||
			(dir == DIRECTION_EAST && Item::items[newBed->id].bedPartnerDir == DIRECTION_WEST)) {
			if (Item::items[newBed->id].bedType == Item::items[id].bedType) {
				return newBed;
			}
		}
	}
...
void BedItem::removeCall() {
	BedItem* nextBedItem = getNextBedItem();
	if (nextBedItem && sleeperGUID != 0){
		nextBedItem->wakeUp(nullptr);
