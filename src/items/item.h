...
class ItemAttributes
{
	...
	private:
		...
		const static uint32_t intAttributeTypes = ITEM_ATTRIBUTE_ACTIONID | ITEM_ATTRIBUTE_UNIQUEID | ITEM_ATTRIBUTE_DATE
			...
			| ITEM_ATTRIBUTE_OPENCONTAINER | ITEM_ATTRIBUTE_QUICKLOOTCONTAINER | ITEM_ATTRIBUTE_DURATION_TIMESTAMP | ITEM_ATTRIBUTE_BOOST;
...
class Item : virtual public Thing
{
    ...
    static std::vector<std::pair<std::string, std::string>> getDescriptions(const ItemType& it,
		...
		uint16_t getWeightReduction() const {
		...
		int8_t getTier() const {
			return items[id].classification;
		}
		int8_t getBoost() const {
			if (hasAttribute(ITEM_ATTRIBUTE_BOOST)) {
				return getIntAttr(ITEM_ATTRIBUTE_BOOST);
			}
			return 0;
		}
