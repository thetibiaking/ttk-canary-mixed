...
uint32_t MoveEvent::EquipItem(MoveEvent* moveEvent, Player* player, Item* item, slots_t slot, bool isCheck)
{
	...
	if (item && it.classification != 0) {
		if (item->getBoost() > 0) {
			if (it.slotPosition & SLOTP_ARMOR) {
				player->setVarSpecial(SPECIAL_RUSE, g_forge.getArmorBonus(it.classification, item->getBoost()));
			} else if (it.slotPosition & SLOTP_HEAD) {
				player->setVarSpecial(SPECIAL_MOMENTUM, g_forge.getHelmetBonus(it.classification, item->getBoost()));
			} else if (it.slotPosition & SLOTP_HAND) {
				player->setVarSpecial(SPECIAL_ONSLAUGHT, g_forge.getWeaponBonus(it.classification, item->getBoost()));
			}
	...
	for (uint16_t combat = 0; combat <= 11; combat++) {
		if (it.abilities->reflectPercent[combat] != 0)
			player->setReflectPercent(indexToCombatType(combat), it.abilities->reflectPercent[combat]);
		
		if (it.abilities->reflectFlat[combat] != 0)
			player->setReflectFlat(indexToCombatType(combat), it.abilities->reflectFlat[combat]);
