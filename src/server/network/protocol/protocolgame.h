...
private:
	...
	void parseOpenForge(NetworkMessage& msg);
	...
	void parsePartyHuntAnalyzer(NetworkMessage& msg);
	...
	void parseHotkeyConfiguration(NetworkMessage& msg);
	...
	void sendMarketBrowseItem(uint16_t itemId, uint8_t tier, const MarketOfferList& buyOffers, const MarketOfferList& sellOffers);
	...
	// Forge
	void sendForgeBasicData();
	void sendExaltationForge();
	void sendExaltedFusionWindow(bool sucess, Item* upgradeItem, uint16_t secondaryItem, uint8_t itemTier, ExaltedForgeBonus_t bonus, uint8_t bonusInfo);
	void sendExaltedTransferWindow(uint16_t oldItemId, uint8_t oldItemTier, Item* newItem);
	void sendForgeHistory();
	...
	void AddExperienceInfo(uint64_t rawExp, uint64_t exp);
