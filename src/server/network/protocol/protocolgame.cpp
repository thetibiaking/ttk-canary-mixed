...
void ProtocolGame::parsePacketFromDispatcher(NetworkMessage msg, uint8_t recvbyte)
{
	...
	switch (recvbyte) {
		...
		case 0x2B: parsePartyHuntAnalyzer(msg); break;
		...
		case 0xC0: parseOpenForge(msg); break;
		...
		case 0xF3: parseHotkeyConfiguration(msg); break;
...
void ProtocolGame::parseHotkeyEquip(NetworkMessage &msg)
{
	...
	//std::cout << "ItemTier (" << static_cast<uint16_t>(tier) << ")" << std::endl;

	addGameTask(&Game::onPressHotkeyEquip, player->getID(), spriteid, tier);
}
	...
	if (knownCreatureSet.size() > 1300) {
		...
		for (auto it = knownCreatureSet.begin(), end = knownCreatureSet.end(); it != end; ++it) {
			...
			if (creature && (checkPlayer = creature->getPlayer()) != nullptr) {
				if (player->getParty() != checkPlayer->getParty() && !canSee(creature) && id != *it) {
					removedKnown = *it;
					knownCreatureSet.erase(it);
					return;
				}
			} else if (!canSee(creature)&& id != *it) {
				removedKnown = *it;
				knownCreatureSet.erase(it);
				return;
			}
...
{
	if (player->getNextForgeAction() > time(nullptr)) {
		player->sendCancelMessage(RETURNVALUE_YOUAREEXHAUSTED);
		return;
	}

	player->nextNextForgeAction();
	
	ForgeDefaultAction_t action = static_cast<ForgeDefaultAction_t>(msg.getByte());
	switch (action) {
		case FORGE_ACTION_FUSION: {
			uint16_t primaryItemId = Item::items.getItemIdByClientId(msg.get<uint16_t>()).id;
			uint8_t itemTier = msg.getByte();
			uint16_t secondaryItemId = Item::items.getItemIdByClientId(msg.get<uint16_t>()).id;

			bool improveSucess = msg.getByte();
			bool reduceLoss = msg.getByte();

			g_forge.processFusion(player, primaryItemId, secondaryItemId, itemTier, improveSucess, reduceLoss);
			break;
		}
		case FORGE_ACTION_TRANSFER: {
			...
			g_forge.processTransfer(player, primaryItemId, secondaryItemId, itemTier);
			break;
		}
		case FORGE_ACTION_DUST: {
			g_forge.convertDust(player);
			break;
		}
		case FORGE_ACTION_SLIVER: {
			g_forge.convertSliver(player);
			break;
		}
		case FORGE_ACTION_LIMIT: {
			g_forge.raiseDustLimit(player);
			break;
		}
		default: break;
	}
...
void ProtocolGame::parseOpenForge(NetworkMessage&)
{
	if (!player) {
		return;
	}
...
void ProtocolGame::parseHotkeyConfiguration(NetworkMessage& msg)
{
	if (!player) {
		return;
	}
	
	std::multimap<uint16_t, uint8_t> hotkeyItemMap;
	uint8_t itemCount = msg.getByte();
	for (int i = 0; i < itemCount; i++) {
		uint16_t itemId = msg.get<uint16_t>();
		uint8_t subType = msg.getByte();
		hotkeyItemMap.emplace(itemId, subType);
	}

	player->setHotkeyItemMap(hotkeyItemMap);
}
...
{
	MarketRequest_t resquest = static_cast<MarketRequest_t>(msg.getByte());
	switch (resquest) {
		case MARKETREQUEST_OWN_HISTORY: {
			addGameTask(&Game::playerBrowseMarketOwnHistory, player->getID());
			break;
		}
		case MARKETREQUEST_OWN_OFFERS: {
			addGameTask(&Game::playerBrowseMarketOwnOffers, player->getID());
			break;
		}
		case MARKETREQUEST_SELECT_ITEM: {
			uint16_t itemId = msg.get<uint16_t>();
			uint8_t tier = 0;

			const ItemType& iType = Item::items.getItemIdByClientId(itemId);
			if (iType.classification != 0) {
				tier = msg.getByte();
			}
...
void ProtocolGame::parseMarketCreateOffer(NetworkMessage &msg)
{
	...
	if (amount > 0 && amount <= 64000 && price > 0 && price <= std::numeric_limits<uint64_t>::max() && (type == MARKETACTION_BUY || type == MARKETACTION_SELL)) {
		addGameTask(&Game::playerCreateMarketOffer, player->getID(), type, spriteId, tier, amount, price, anonymous);
	}
}
...
void ProtocolGame::parseMarketAcceptOffer(NetworkMessage &msg)
{
	...
	if (amount > 0 && amount <= 64000) {
		addGameTask(&Game::playerAcceptMarketOffer, player->getID(), timestamp, offerId, amount);
	}
	updateCoinBalance();
}
	...
	if (!creature)
 	...
 	msg.addByte(0x8B);
  	...
  	if (icon != CREATUREICON_NONE) {
    	msg.addByte(icon);
    	msg.addByte(1);
			msg.add<uint16_t>(creature->isInfluenced() ? creature->getInfluencedStack() : 0x00);
  	}
	writeToOutputBuffer(msg);
}
...
void ProtocolGame::sendCyclopediaCharacterCombatStats()
{
	...
	msg.addDouble(player->getMitigation()); // Version 13.10 Migitation
	...
	for (uint16_t i = 0; i < COMBAT_COUNT; ++i) {
		absorbs[i] += player->getWheelOfDestinyResistance(indexToCombatType(i)) / 100.;
	}
...
void ProtocolGame::sendCyclopediaCharacterItemSummary()
{
	...
	std::unordered_map<uint16_t, uint32_t> inventory = player->getInventoryItems();
	msg.add<uint16_t>(inventory.size());
	for (auto inventoryItem : inventory) {
		msg.add<uint16_t>(inventoryItem.first);
		if (Item::items.getItemIdByClientId(inventoryItem.first).classification != 0) {
			msg.addByte(0x00);
		}
		msg.add<uint32_t>(inventoryItem.second);
	}

	std::unordered_map<uint16_t, uint32_t> storeInbox = player->getStoreInboxItems();
	msg.add<uint16_t>(storeInbox.size());
	for (auto storeItem : storeInbox) {
		msg.add<uint16_t>(storeItem.first);
		if (Item::items.getItemIdByClientId(storeItem.first).classification != 0) {
			msg.addByte(0x00);
		}
		msg.add<uint32_t>(storeItem.second);
	}

	StashItemList stashItems = player->getStashItems();
	msg.add<uint16_t>(stashItems.size());
	for (auto stashItem : stashItems) {
		msg.add<uint16_t>(stashItem.first);
		msg.add<uint32_t>(stashItem.second);
	}

	std::unordered_map<uint16_t, uint32_t> depotItems = player->getDepotItems();
	msg.add<uint16_t>(depotItems.size());
	for (auto depotItem : depotItems) {
		msg.add<uint16_t>(depotItem.first);
		if (Item::items.getItemIdByClientId(depotItem.first).classification != 0) {
			msg.addByte(0x00);
		}
		msg.add<uint32_t>(depotItem.second);
	}

	std::unordered_map<uint16_t, uint32_t> inboxItems = player->getDepotInboxItems();
	msg.add<uint16_t>(inboxItems.size());
	for (auto inboxItem : inboxItems) {
		msg.add<uint16_t>(inboxItem.first);
		if (Item::items.getItemIdByClientId(inboxItem.first).classification != 0) {
			msg.addByte(0x00);
		}
		msg.add<uint32_t>(inboxItem.second);
	}
...
void ProtocolGame::sendMarketEnter(uint32_t depotId)
{
	...
	do {
		...
		for (Item* item : container->getItemList()) {
			...
			for (auto checkItem : itemList) {
				if (checkItem->getID() == item->getID()) {
					if (checkItem->getBoost() == item->getBoost()) {
						itemFound = true;
					}
				}
			}
	...
	msg.add<uint16_t>(itemList.size() + player->stashItems.size());
	for (auto it : itemList) {
		msg.add<uint16_t>(it->getClientID());
		if (it->getTier() != 0) {
			msg.addByte(it->getBoost());
		}

		msg.add<uint16_t>(player->getDepotItemCountByTier(it->getID(), it->getBoost()));
	}

	for (auto it : player->stashItems) {
...
void ProtocolGame::sendMarketBrowseItem(uint16_t itemId, uint8_t tier, const MarketOfferList& buyOffers, const MarketOfferList& sellOffers)
{
	NetworkMessage msg;
	msg.addByte(0xF9);

	msg.addByte(MARKETREQUEST_SELECT_ITEM);
...
void ProtocolGame::sendMarketBrowseOwnOffers(const MarketOfferList &buyOffers, const MarketOfferList &sellOffers)
{
	...
	for (const MarketOffer& offer : buyOffers) {
		...
		msg.add<uint16_t>(offer.counter);
		msg.addItemId(offer.itemId);
		if (Item::items.getItemType(offer.itemId).classification != 0) {
			msg.addByte(static_cast<uint16_t>(offer.tier));
		}
		msg.add<uint16_t>(offer.amount);
		msg.add<uint64_t>(offer.price);
	}
...
void ProtocolGame::sendMarketBrowseOwnHistory(const HistoryMarketOfferList &buyOffers, const HistoryMarketOfferList &sellOffers)
{
	...
	for (auto it = buyOffers.begin(); i < buyOffersToSend; ++it, ++i) {
		...
		if (Item::items.getItemType(it->itemId).classification != 0) {
			msg.addByte(it->tier);
		}
...
void ProtocolGame::sendMarketDetail(uint16_t itemId, uint8_t tier)
{
	...
	if (version >= 1270) {
		...
		if (it.classification != 0) {
			msg.addString(std::to_string(it.classification));
...
void ProtocolGame::sendItems(const std::multimap<uint16_t, uint8_t>& inventoryMap)
{
	NetworkMessage msg;
	msg.addByte(0xF5);

	msg.add<uint16_t>(inventoryMap.size());
	for (const auto& it : inventoryMap) {
		msg.addItemId(it.first);
		if (Item::items.getItemType(it.first).isFluidContainer()) {
			uint32_t index = static_cast<uint32_t>(it.second) << 16;
			uint8_t fluidType = static_cast<uint8_t>(index >> 16);
			msg.addByte(serverFluidToClient(fluidType));
			msg.add<uint16_t>(player->getItemTypeCount(it.first, fluidType));
		} else {
			msg.addByte(it.second);
			msg.add<uint16_t>(player->getItemTypeCountByTier(it.first, it.second, true));
		}
	}
...
void ProtocolGame::AddPlayerSkills(NetworkMessage &msg)
{
	...
	if (version >= 1280) {
		for (uint8_t special = SPECIAL_FIRST; special <= SPECIAL_LAST; ++special) {
			msg.add<uint16_t>(player->varSpecial[special] * 100);
			msg.add<uint16_t>(player->varSpecial[special] * 100);
		}
	}
...
void ProtocolGame::AddExperienceInfo(uint64_t rawExp, uint64_t exp)
{
	NetworkMessage msg;
	msg.addByte(0xAF);
	msg.add<uint64_t>(rawExp);
	msg.add<uint64_t>(exp);
	writeToOutputBuffer(msg);
}
...
void ProtocolGame::sendImbuementWindow(Item *item)
{
	...
	if (Item::items.getItemType(item->getID()).classification != 0) {
		msg.addByte(0x00);
	}
...
void ProtocolGame::parsePartyHuntAnalyzer(NetworkMessage& msg)
{
	uint8_t type = msg.getByte();
	if (type == TYPE_CLEAR) {
		if (Party* party = player->getParty()) {
			party->clearInformation();
		}
	} else if (type == TYPE_CHANGE_MODE) {
		uint8_t lootType = msg.getByte();
		if (Party* party = player->getParty()) {
			if (lootType == 0) {
				party->setPartyLootMode(lootType);
			} else {
				party->setPartyLootMode(lootType);
			}
		}
	} else if (type == TYPE_CYCLOPEDIA_PRICE) {
		...
		uint16_t itemCount = msg.get<uint16_t>();
		for (int i = 0; i < itemCount; i++) {
			uint16_t itemId = msg.get<uint16_t>();
			uint64_t itemPrice = msg.get<uint64_t>();
			leaderList[itemId] = itemPrice;
		}

		party->updateLeaderItems(leaderList);
	}
}
...
{
	std::multimap<uint16_t, MarketStatisticInfo> marketStatistics = g_game.getMarketStatistics();

	NetworkMessage msg;
	msg.addByte(0xCD);
	msg.add<uint16_t>(marketStatistics.size());
	for (const auto& it : marketStatistics) {
		msg.addItemId(it.first);
		if (Item::items.getItemType(it.first).classification != 0) {
			msg.addByte(it.second.tier);
		}
		msg.add<uint64_t>(it.second.price);
	}
...
void ProtocolGame::updatePartyHuntStatus()
{
	Party* party = player->getParty();
	if (!party) {
		clearPartyHuntStatus();
		return;
	}
	...
	msg.add<uint32_t>(party->getCreateTime());
	msg.add<uint32_t>(party->getLeader()->getID());

	msg.addByte(party->getLootMode());

	ActiveVector activeMembers = party->getActiveMembers();
	PartyMap statistic = party->getInformation();

	msg.addByte(activeMembers.size());
	for (uint32_t playerId : activeMembers) {
		Player* member = g_game.getPlayerByGUID(playerId);
		if (member) {
			msg.add<uint32_t>(member->getID());
			msg.addByte(statistic[playerId].active);
			msg.add<uint64_t>(party->getLootBalanceByPlayer(playerId));
			msg.add<uint64_t>(statistic[playerId].supplyWaste);
			msg.add<uint64_t>(statistic[playerId].damageDealt);
			msg.add<uint64_t>(statistic[playerId].incomingHeal);
		} else {
	...
	msg.addByte(activeMembers.size() != 0 ? 1 : 0);
	if (activeMembers.size() != 0) {
		...
		for (uint32_t playerId : activeMembers) {
			...
			if (member) {
				...
				msg.add<uint32_t>(0);
				msg.addString(IOLoginData::getNameByGuid(playerId));
			}
		}
	}
...
void ProtocolGame::clearPartyHuntStatus()
{
	NetworkMessage msg;
	msg.addByte(0x2B);
	msg.add<uint32_t>(0);
...
void ProtocolGame::sendForgeBasicData()
{
	NetworkMessage msg;
	msg.addByte(0x86);
	std::map<uint16_t, std::vector<ExaltedCategory>> categories = g_forge.getCategories();
	...
	for (auto it : categories) {
		...
		for (auto info : it.second) {
			msg.addByte(info.tier);
			msg.add<uint64_t>(info.price);
		}
	}

	msg.addByte(g_forge.getExaltedConfig().dustPercent);
	msg.addByte(g_forge.getExaltedConfig().dustToSleaver);
	msg.addByte(g_forge.getExaltedConfig().sliverToCore);
	msg.addByte(g_forge.getExaltedConfig().dustPercentUpgrade);
	msg.addByte(player->getMaxDust());
	msg.addByte(g_forge.getExaltedConfig().maxDustCap);
	msg.addByte(g_forge.getExaltedConfig().dustFusion);
	msg.addByte(g_forge.getExaltedConfig().dustTransfer);
	msg.addByte(g_forge.getExaltedConfig().chanceBase);
	msg.addByte(g_forge.getExaltedConfig().chanceImproved);
	msg.addByte(g_forge.getExaltedConfig().reduceTierLoss);

	writeToOutputBuffer(msg);
}

void ProtocolGame::sendExaltationForge()
{
	NetworkMessage msg;
	msg.addByte(0x87);

	std::multimap<uint16_t, uint8_t> itemList = player->getFusedItems();
	msg.add<uint16_t>(itemList.size());
	for (auto it : itemList) {
		msg.addByte(0x01);
		const ItemType& iType = Item::items.getItemIdByClientId(it.first);
		msg.add<uint16_t>(iType.clientId);
		msg.addByte(it.second);
		msg.add<uint16_t>(player->getItemTypeCountByTier(iType.id, it.second));
	}

	std::map<Item*, std::map<uint16_t, uint16_t>> transferList = player->getTransferItems();

	msg.addByte(transferList.size());
	for (auto ite : transferList) {
		msg.add<uint16_t>(1);
		msg.add<uint16_t>(ite.first->getClientID());
		msg.addByte(ite.first->getBoost());
		msg.add<uint16_t>(player->getItemTypeCountByTier(ite.first->getID(), ite.first->getBoost()));

		msg.add<uint16_t>(ite.second.size());
		for (auto list : ite.second) {
			msg.add<uint16_t>(list.first);
			msg.add<uint16_t>(list.second);
		}
	}
...
void ProtocolGame::sendExaltedFusionWindow(bool sucess, Item* upgradeItem, uint16_t secondaryItem, uint8_t itemTier, ExaltedForgeBonus_t bonus, uint8_t bonusInfo)
{
	if (!upgradeItem) {
		return;
	}
	...
	msg.addByte(0x00);
	msg.addByte(sucess);
	msg.add<uint16_t>(upgradeItem->getClientID());
	msg.addByte(itemTier);
	msg.add<uint16_t>(Item::items.getItemType(secondaryItem).clientId);
	msg.addByte(upgradeItem->getBoost() == 0 ? 1 : upgradeItem->getBoost());
	msg.addByte(bonus);

	if (bonus == BONUS_CORE) {
		msg.addByte(bonusInfo);
	} else if (bonus == BONUS_KEEP_TIER_SUCESS || bonus == BONUS_KEEP_ITEM_TIER || bonus == BONUS_BOTH_TIER_UPGRADE || bonus == BONUS_DOUBLE_TIER_UPGRADE || bonus == BONUS_KEEP_TIER_FAIL) {
		msg.add<uint16_t>(Item::items.getItemType(secondaryItem).clientId);
		msg.addByte(bonusInfo);
	}
...
void ProtocolGame::sendExaltedTransferWindow(uint16_t oldItemId, uint8_t oldItemTier, Item* newItem)
{
	if (!newItem) {
		return;
	}
	...
	msg.add<uint16_t>(Item::items.getItemType(oldItemId).clientId);
	msg.addByte(oldItemTier);
	msg.add<uint16_t>(newItem->getClientID());
	msg.addByte(newItem->getBoost());
	msg.addByte(0x00);

	writeToOutputBuffer(msg);
}

void ProtocolGame::sendForgeHistory()
{
	NetworkMessage msg;
	msg.addByte(0x88);
	msg.add<uint16_t>(0x00);
	msg.add<uint16_t>(0x01);
	msg.addByte(player->forgeHistory.size());
	for (auto info : player->forgeHistory) {
		msg.add<uint32_t>(info.time);
		msg.addByte(info.action);
		msg.addString(info.message);
		msg.addByte(info.error);
	}

	writeToOutputBuffer(msg);
}

void ProtocolGame::sendHotkeyItems()
{
	const std::multimap<uint16_t, uint8_t> items = player->getHotkeyItemMap();
	...
	for (auto it : items) {
		msg.add<uint16_t>(it.first);
		msg.addByte(it.second);
		std::string itemName = Item::items.getItemIdByClientId(it.first).name;
