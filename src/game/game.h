...
class Game
{
	public:
		...
		void playerBuyItem(uint32_t playerId, uint16_t spriteId, uint8_t count, uint32_t amount,
								bool ignoreCap = false, bool inBackpacks = false);
		void playerSellItem(uint32_t playerId, uint16_t spriteId, uint8_t count,
								uint32_t amount, bool ignoreEquipped = false);
		...
		void playerCreateMarketOffer(uint32_t playerId, uint8_t type, uint16_t spriteId, uint8_t tier, uint16_t amount, uint64_t price, bool anonymous);
		...
		std::forward_list<Item*> getMarketItemList(uint16_t wareId, uint8_t tier, uint16_t sufficientCount, DepotLocker* depotLocker);
		...
		void onPressHotkeyEquip(uint32_t playerId, uint16_t spriteid, uint8_t tier);
		...
		std::multimap<uint16_t, MarketStatisticInfo>& getMarketStatistics() {
			return marketAveragePrice;
		}

		void checkMarketStatistics();
		uint64_t getItemAveragePrice(uint16_t itemId, uint8_t tier);
		...
		void spawnFiendishMonster(uint32_t clearId = 0);
		std::unordered_map<uint32_t, Monster*> getFiendishMonsters() const {
	...
	private:
		...
		std::multimap<uint16_t, MarketStatisticInfo> marketAveragePrice;
		...
		std::unordered_map<uint32_t, Monster*> fiendishMonsters;
