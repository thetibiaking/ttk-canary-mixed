...
Task* createOldTask(std::function<void (void)> f)
{
	return new Task(std::move(f), "luaMapLoadPart", "luaMapLoadPart");
}

Task* createOldTask(uint32_t expiration, std::function<void (void)> f)
{
	return new Task(expiration, std::move(f), "luaMapLoadPart", "luaMapLoadPart");
}
