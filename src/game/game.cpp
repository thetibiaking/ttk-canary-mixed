...
void Game::setGameState(GameState_t newState)
{
	...
	switch (newState) {
		case GAME_STATE_INIT: {
			...
			for (uint32_t i = 0; i < g_forge.getMonsterConfig().maxFiendishCount; i++) {
				spawnFiendishMonster();
			}
			break;
		}
...
void Game::onPressHotkeyEquip(uint32_t playerId, uint16_t spriteid, uint8_t tier)
{
	...
	if (itemType.weaponType == WEAPON_AMMO) {
		...
		item = findItemOfType(player, itemType.id, true, -1, tier);
...
bool Game::removeCreature(Creature* creature, bool isLogout/* = true*/)
{
	...
	if (creature->isFiendish()) {
		spawnFiendishMonster(creature->getID());
	}
						  ...
						  uint16_t spriteId, uint8_t fromStackPos, const Position& toPos, uint8_t count, Item* item, Cylinder* toCylinder)
{
	...
	if (ret != RETURNVALUE_NOERROR) {
		...
		if (party) {
			party->updateInformation(player, INFO_LOOT, item, 0);
		}
	}
	player->cancelPush();
...
Item* Game::findItemOfType(Cylinder* cylinder, uint16_t itemId,
						   bool depthSearch /*= true*/, int32_t subType /*= -1*/, uint8_t tier /*= 0*/) const
{
	if (cylinder == nullptr) {
		return nullptr;
	}
...
void Game::playerWrapableItem(uint32_t playerId, const Position& pos, uint8_t stackPos, const uint16_t spriteId)
{
	...
	// if (BedItem* bedItem = item->getBed()){
	  // bedItem->removeFromHouse();
  // }
	...
	if (item->isWrapable() && !item->isWrapBox()) {
	  ...
	  newItem->startDecaying();
		...
		if (newItem) {
			...
			newItem->startDecaying();
			if (BedItem* bedItem = newItem->getBed()){
				house->addBed(bedItem);
			}
	  }
  }
}
...
void Game::playerBuyItem(uint32_t playerId, uint16_t spriteId, uint8_t count, uint32_t amount,
...
void Game::playerSellItem(uint32_t playerId, uint16_t spriteId, uint8_t count, uint32_t amount, bool ignoreEquipped)
{
	if (amount == 0) {
		return;
	}
...
bool Game::internalCreatureSay(Creature* creature, SpeakClasses type, const std::string& text,
							   bool ghostMode, SpectatorHashSet* spectatorsPtr/* = nullptr*/, const Position* pos/* = nullptr*/, bool playerDisplay /* = true*/)
{
	if (text.empty()) {
		return false;
	}
	...
	for (Creature* spectator : spectators) {
		if (Player* tmpPlayer = spectator->getPlayer()) {
			if (!playerDisplay && tmpPlayer == creature) {
				continue;
			}
...
bool Game::combatBlockHit(CombatDamage& damage, Creature* attacker, Creature* target, bool checkDefense, bool checkArmor, bool field)
{
	...
	if (!damage.extension && damage.primary.type != COMBAT_NONE && damage.primary.type != COMBAT_HEALING) {
		// Damage reflection primary
		if (attacker) {
			int32_t primaryReflectPercent = target->getReflectPercent(damage.primary.type);
			int32_t primaryReflectFlat = target->getReflectFlat(damage.primary.type);
			if (primaryReflectPercent > 0 || primaryReflectFlat > 0) {
				damageReflected.primary.value = std::ceil(damage.secondary.value * primaryReflectPercent / 100.) + (-static_cast<int64_t>(std::ceil(attacker->getMaxHealth() * 0.01)), (damage.secondary.value, -(static_cast<int64_t>(primaryReflectFlat))));
	...
	if (!damage.extension && damage.secondary.type != COMBAT_NONE && damage.primary.type != COMBAT_HEALING) {
		...
		if (attacker) {
			...
			if (secondaryReflectPercent > 0 || secondaryReflectFlat > 0) {
				if (!canReflect) {
					...
					damageReflected.primary.value = std::ceil(damage.secondary.value * secondaryReflectPercent / 100.) + (-static_cast<int64_t>(std::ceil(attacker->getMaxHealth() * 0.01)), (damage.secondary.value, -(static_cast<int64_t>(secondaryReflectFlat))));
					damageReflected.extension = true;
					damageReflected.exString = "(damage reflection)";
					canReflect = true;
				} else {
					damageReflected.secondary.type = damage.secondary.type;
					damageReflected.secondary.value = std::ceil(damage.secondary.value * secondaryReflectPercent / 100.) + (int64_t(std::ceil(attacker->getMaxHealth() * 0.01)), (damage.secondary.value, -(static_cast<int64_t>(secondaryReflectFlat))));
				}
			}
		}
...
bool Game::combatChangeHealth(Creature* attacker, Creature* target, CombatDamage& damage, bool isEvent /*= false*/)
{
	...
	if (damage.primary.value > 0) {
		...
		if (realHealthChange > 0 && !target->isInGhostMode()) {
			...
			if (targetPlayer && targetPlayer->getParty()) {
				Party* party = targetPlayer->getParty();
				if (party) {
					party->updateInformation(targetPlayer, INFO_HEALING, nullptr, realHealthChange);
				}
			}
		...
		if (damage.origin != ORIGIN_NONE && attacker && damage.primary.type != COMBAT_HEALING) {
			...
			damage.primary.value *= ((attacker->getBuff(BUFF_DAMAGEDEALT) / 100.) + (attacker->getBuff(BUFF_INFLUENCED_DAMAGEDEALT) / 100.));
			damage.secondary.value *= ((attacker->getBuff(BUFF_DAMAGEDEALT) / 100.) + (attacker->getBuff(BUFF_INFLUENCED_DAMAGEDEALT) / 100.));
		}
		if (damage.origin != ORIGIN_NONE && target && damage.primary.type != COMBAT_HEALING) {
			...
			damage.primary.value *= ((target->getBuff(BUFF_DAMAGERECEIVED) / 100.) + (target->getBuff(BUFF_INFLUENCED_DAMAGERECEVIED) / 100.));
			damage.secondary.value *= ((target->getBuff(BUFF_DAMAGERECEIVED) / 100.) + (target->getBuff(BUFF_INFLUENCED_DAMAGERECEVIED) / 100.));
		}
		...
		if (!damage.extension && attackerMonster && targetPlayer) {
			...
			if (charmRune_t activeCharm = g_bestiary.getCharmFromTarget(targetPlayer, g_monsters.getMonsterTypeByRaceId(attackerMonster->getRaceId()));
				activeCharm != CHARM_NONE && activeCharm != CHARM_CLEANSE && damage.canTrigerCharm == true) {
					
				Charm* charm = g_bestiary.getBestiaryCharm(activeCharm);
				if (charm) {
					int8_t charmChance = charm->chance;
					Condition* protection = targetPlayer->getCondition(CONDITION_SPECIALPOTION_EFFECT, CONDITIONID_DEFAULT, POTION_CHARM);
					if (protection) {
						charmChance += 5;
					}
					if (charm->type == CHARM_DEFENSIVE && charm->chance > normal_random(0, 100) && 
		...
		if (targetPlayer && targetPlayer->getWheelOfDestinyInstant("Gift of Life") && targetPlayer->getWheelOfDestinyGiftOfCooldown() == 0 && 
		...
		if (attackerPlayer && target->getMonster()) {
			if (Party* party = attackerPlayer->getParty()) {
				if (!party->hasInformation) {
					party->hasInformation = true;
				}
				party->updateInformation(attackerPlayer, INFO_DAMAGE, nullptr, damage.primary.value + damage.secondary.value);
			}
		}
		...
		if (attackerPlayer && !damage.cleave && !damage.extension) {
			...
			if (normal_random(0, 100) < lifeChance && damage.canTrigerLeach) {
				// Vampiric charm rune
				if (targetMonster) {
					if (uint16_t playerCharmRaceidVamp = attackerPlayer->parseRacebyCharm(CHARM_VAMP, false, 0); 
						playerCharmRaceidVamp != 0 && playerCharmRaceidVamp == targetMonster->getRaceId() && damage.canTrigerCharm) {
						if (const Charm* lifec = g_bestiary.getBestiaryCharm(CHARM_VAMP)) {
				...
				// WARNING Wheel of Destiny changes
				tmpDamage.primary.value = std::round((realDamage * (0.9 + (damage.affected / 10.)) * (attackerPlayer->getSkillLevel(SKILL_LIFE_LEECH_AMOUNT) /10000.)) / damage.affected);
			...
			if (normal_random(0, 100) < manaChance && damage.canTrigerLeach) {
				// Void charm rune
				if (targetMonster) {
					if (uint16_t playerCharmRaceidVoid = attackerPlayer->parseRacebyCharm(CHARM_VOID, false, 0);
						playerCharmRaceidVoid != 0 && playerCharmRaceidVoid == targetMonster->getRace() && damage.canTrigerCharm) {
						if (const Charm* voidc = g_bestiary.getBestiaryCharm(CHARM_VOID)) {
			...
			//	tmpDamage.primary.value = std::round(realDamage * (manaSkill / 100.) * (0.1 * affected + 0.9)) / affected;
				tmpDamage.primary.value = std::round((realDamage * (0.9 + (damage.affected / 10.)) * (attackerPlayer->getSkillLevel(SKILL_MANA_LEECH_AMOUNT) /10000.)) / damage.affected);
			...
			if (!damage.extension && targetMonster) {
				if (charmRune_t activeCharm = g_bestiary.getCharmFromTarget(attackerPlayer, g_monsters.getMonsterTypeByRaceId(targetMonster->getRaceId()));
					activeCharm != CHARM_NONE && damage.canTrigerCharm) {
					Charm* charm = g_bestiary.getBestiaryCharm(activeCharm);
					if (charm) {
						int8_t charmChance = charm->chance;
						Condition* protection = attackerPlayer->getCondition(CONDITION_SPECIALPOTION_EFFECT, CONDITIONID_DEFAULT, POTION_CHARM);
						if (protection) {
							charmChance += 5;
						}
						if (charm->type == CHARM_OFFENSIVE && (charm->chance >= normal_random(0, 100))) {
		...
		if (attacker && target && targetPlayer) {
			if (targetPlayer->varSpecial[SPECIAL_RUSE] != 0) {
				double chance = uniform_double_random();
				if (chance <= targetPlayer->varSpecial[SPECIAL_RUSE]) {
					addMagicEffect(spectators, targetPos, CONST_ME_DODGE);
					damage.primary.value = 0;
					damage.secondary.value = 0;
					targetPlayer->sendTextMessage(MESSAGE_FAILURE, "You dodged an attack. (Ruse)");
				}
			}
		}
	...
	if (manaChange > 0) {
		...
		if (targetPlayer && attacker && attacker->getMonster()) {
			...
			if (mType) {;
				...
				if (activeCharm != CHARM_NONE && activeCharm != CHARM_CLEANSE && damage.canTrigerCharm == true) {
					Charm* charm = g_bestiary.getBestiaryCharm(activeCharm);
					if (charm) {
						int8_t charmChance = charm->chance;
						Condition* protection = targetPlayer->getCondition(CONDITION_SPECIALPOTION_EFFECT, CONDITIONID_DEFAULT, POTION_CHARM);
...
void Game::updateCreatureEmblem(Creature* creature)
{
	SpectatorHashSet spectators;
	map.getSpectators(spectators, creature->getPosition(), true, true);
	creature->getPlayer()->guildWarVector.clear();
	if (creature->getPlayer()->getGuild())
		IOGuild::getWarList(creature->getPlayer()->getGuild()->getId(), creature->getPlayer()->guildWarVector);
	for (Creature* spectator : spectators) {
		spectator->getPlayer()->guildWarVector.clear();
		if (spectator->getPlayer()->getGuild())
			IOGuild::getWarList(spectator->getPlayer()->getGuild()->getId(), spectator->getPlayer()->guildWarVector);
...
void Game::playerBrowseMarket(uint32_t playerId, uint16_t spriteId, uint8_t tier)
{
	...
	player->sendMarketBrowseItem(it.id, tier, buyOffers, sellOffers);
...
void Game::playerCreateMarketOffer(uint32_t playerId, uint8_t type, uint16_t spriteId, uint8_t tier, uint16_t amount, uint64_t price, bool anonymous) // Limit of 64k of items to create offer
{
	// 64000 is size of the client limitation
	if (amount == 0 || amount > 64000 || price == 0) {
		return;
	}
	...
	if (player->operatingSystem != CLIENTOS_NEW_WINDOWS) {
		SPDLOG_ERROR("[void Game::playerCreateMarketOffer] - Player com nome {} tentou executar uma oferta no market com cliente diferente do oficial.", player->getName());
		return;
	}
	...
	if (type == MARKETACTION_SELL) {
      ...
      account.RemoveCoins(static_cast<uint32_t>(amount));
		...
		if (stashmath > 0) {
			...
			std::forward_list<Item *> itemList = getMarketItemList(it.wareId, tier, stashmath, depotLocker);

			uint16_t tempTest = stashmath;

				if (it.stackable) {
					for (Item *item : itemList) {
						uint16_t removeCount = std::min<uint16_t>(tempTest, item->getItemCount());
						tempTest -= removeCount;
					}
				} else {
					for (Item *item : itemList) {
						tempTest--;
				...
				if (tempTest > 0) {
					SPDLOG_ERROR("[Void Game::playerCreateMarketOffer] - player with name {} not have item with id {}", player->getName(), it.id);
					return;
				}
	...
	//SPDLOG_ERROR("Checking Type offer {}", static_cast<MarketAction_t>(type));
		
	
	IOMarket::createOffer(player->getGUID(), static_cast<MarketAction_t>(type), it.id, tier, amount, price, anonymous);
	...
	if (offer.type == MARKETACTION_BUY) {
      ...
      account.AddCoins(offer.amount);
			...
			for (uint16_t i = 0; i < offer.amount; ++i) {
				...
				if (item->getTier() != 0) {
					item->setIntAttr(ITEM_ATTRIBUTE_BOOST, offer.tier);
				}
				if (internalAddItem(player->getInbox(), item, INDEX_WHEREEVER, FLAG_NOLIMIT) != RETURNVALUE_NOERROR) {
					delete item;
					break;
...
void Game::playerAcceptMarketOffer(uint32_t playerId, uint32_t timestamp, uint16_t counter, uint16_t amount) // Limit of 64k of items to create offer
{
	...
	if (offer.type == MARKETACTION_BUY) {
		...
		if (it.id == ITEM_STORE_COIN) {
			...
			std::forward_list<Item*> itemList = getMarketItemList(it.wareId, offer.tier, amount, depotLocker);
			if (itemList.empty()) {
				return;
			}
	...
	IOMarket::appendHistory(offer.playerId, offer.type, offer.itemId, offer.tier, amount, offer.price, offer.timestamp + marketOfferDuration, OFFERSTATE_ACCEPTED);
	IOMarket::appendHistory(player->getGUID(), (offer.type == MARKETACTION_BUY ? MARKETACTION_SELL : MARKETACTION_BUY), offer.itemId, offer.tier, amount, offer.price, offer.timestamp + marketOfferDuration, OFFERSTATE_ACCEPTEDEX);
...
std::forward_list<Item*> Game::getMarketItemList(uint16_t wareId, uint8_t tier, uint16_t sufficientCount, DepotLocker* depotLocker)
{
...
void Game::checkMarketStatistics()
{
	// Moved from protocolgame.cpp to better handle
	...
	for (auto& it : saleStatistics) {
		uint16_t itemId = it.first;
		MarketStatistics* statistics = &it.second;
		uint64_t avgPrice = (statistics->totalPrice / statistics->numTransactions);
		uint64_t division = 0;

		auto ite = purchaseStatistics.find(itemId);
		if (ite != purchaseStatistics.end()) {
			statistics = &ite->second;
			avgPrice += (statistics->totalPrice / statistics->numTransactions);
			division = 1;
		}

		MarketStatisticInfo info;
		info.tier = it.second.itemTier;
		info.price = (avgPrice >> division);

		marketAveragePrice.emplace(itemId, info);
	}
}

uint64_t Game::getItemAveragePrice(uint16_t itemId, uint8_t tier)
{
	if (itemId == 0) {
		return 0;
	}

	for (const auto& it : marketAveragePrice) {
		if (it.first == itemId && it.second.tier == tier) {
...
void Game::spawnFiendishMonster(uint32_t clearId /* = 0*/)
{
	if (clearId != 0) {
		fiendishMonsters.erase(clearId);
	}

  Start:

	uint32_t id = uniform_random(1, monsters.size());
	auto it = monsters.begin();
	std::advance(it, id);

	Monster* monster = getMonsterByID(it->first);
	if (!monster || monster->isRemoved() || monster->isInfluenced()) {
		goto Start;
	}

	std::map<uint16_t, std::string> bestiaryMonsters = g_game.getBestiaryList();
	auto bestiary = bestiaryMonsters.find(monster->getRaceId());
	if (bestiary == bestiaryMonsters.end()) {
		goto Start;
	}

	if (!monster->becomeStronger(true)) {
		goto Start;
	}

	fiendishMonsters[monster->getID()] = monster;
}
