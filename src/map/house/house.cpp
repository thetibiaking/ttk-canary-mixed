...
void House::setOwner(uint32_t guid, bool updateDatabase/* = true*/, Player* player/* = nullptr*/)
{
	...
	if (owner != 0) {
		...
		auto it = bedsList.begin();
		while (it != bedsList.end()) {
			removeBed(*it);
			it = bedsList.begin();
		}
...
bool House::kickPlayer(Player* player, Player* target)
{
	...
	if ((getHouseAccessLevel(player) <= getHouseAccessLevel(target) || target->hasFlag(PlayerFlag_CanEditHouses)) && player != target) {
		return false;
	}
...
{14329, 1615}, {16075, 16020}, {16099, 16098}, {20254, 20295}, {20255, 20297}, {20257, 20299},
	...
	for (HouseTile* tile : houseTiles) {
		if (const TileItemVector* items = tile->getItemList()) {
			...
			//std::cout << "[Cheking Code] - itype: " << iType.isBed() << " itembed:" << item->getBed() << " itemid:" << item->getID() << " house:" << houseByTile << "\n";
					if (item->isWrapable()) {
