...
class Creature : virtual public Thing
{
    ...
    void setBuff(buffs_t buff, int32_t modifier) {
      varBuffs[buff] += modifier;
		...
		virtual int32_t getReflectPercent(CombatType_t combatType) const { return 0; }
		virtual int32_t getReflectFlat(CombatType_t combatType) const { return 0; }							
		...
		bool isInfluenced() const {
			return influenced;
		}
		bool isFiendish() const {
			return fiendish;
		}
		uint16_t getInfluencedStack() const {
			return influenceStacks;
		}
