...
ReturnValue Combat::canDoCombat(Creature* attacker, Creature* target)
{
	if (attacker) {
		...
		if (g_game.getWorldType() == WORLD_TYPE_NO_PVP) {
			...
			if (attackerPlayer && targetPlayer && attackerPlayer->isInWar(targetPlayer)) {
				isInWar = true;
			}

			if (attacker->getPlayer() || (attackerMaster && attackerMaster->getPlayer())) {
				if (target->getPlayer()) {
					if (!isInWar && !isInPvpZone(attacker, target)) {
						return RETURNVALUE_YOUMAYNOTATTACKTHISPLAYER;
					}
				}
				...
				if (playerCharmRaceid != 0) {
					...
					if (mType && playerCharmRaceid == mType->info.raceid) {
						...
						if (charm) {
							...
							Condition* protection = player->getCondition(CONDITION_SPECIALPOTION_EFFECT, CONDITIONID_DEFAULT, POTION_CHARM);
							if (protection) {
								charmChance += 5;
							}

							if (charmChance > normal_random(0, 100)) {
...
void Combat::combatTileEffects(const SpectatorHashSet& spectators, Creature* caster, Tile* tile, const CombatParams& params)
{
	...
	if (params.soundImpactEffect != SOUND_EFFECT_TYPE_SILENCE) {
		g_game.sendDoubleSoundEffect(tile->getPosition(), params.soundCastEffect, params.soundImpactEffect, caster);
	}
	else if (params.soundCastEffect != SOUND_EFFECT_TYPE_SILENCE) {
...
void Combat::doCombatHealth(Creature* caster, Creature* target, CombatDamage& damage, const CombatParams& params)
{
	...
	if(caster && caster->getPlayer()){
		...
		Player* player = caster->getPlayer();
		if (player->varSpecial[SPECIAL_ONSLAUGHT] != 0) {
			double faltalChance = uniform_double_random();
			if (faltalChance <= player->varSpecial[SPECIAL_ONSLAUGHT]) {
				damage.primary.value += damage.primary.value * 60 / 100;
				damage.secondary.value += damage.secondary.value * 60 / 100;
				damage.fatal = true;
			}
		}
	}
	if (canCombat) {
		...
		if (target && params.soundImpactEffect != SOUND_EFFECT_TYPE_SILENCE) {
			g_game.sendDoubleSoundEffect(target->getPosition(), params.soundCastEffect, params.soundImpactEffect, caster);
		}
		else if (target && params.soundCastEffect != SOUND_EFFECT_TYPE_SILENCE) {
...
void Combat::doCombatMana(Creature* caster, Creature* target, CombatDamage& damage, const CombatParams& params)
{
	...
	if (canCombat) {
		if (caster != nullptr && target != nullptr && params.distanceEffect != CONST_ANI_NONE) {
			addDistanceEffect(caster, caster->getPosition(), target->getPosition(), params.distanceEffect);
		}
...
void ValueCallback::getMinMaxValues(Player* player, CombatDamage& damage, bool useCharges) const
{
	...
	switch (type) {
		case COMBAT_FORMULA_LEVELMAGIC: {
			...
			lua_pushnumber(L, magicLevelSkill + player->getSpecializedMagicLevel(damage.primary.type));
			parameters += 2;
			break;
		}
...
void MagicField::onStepInField(Creature* creature)
{
	...
	if (it.conditionDamage) {
		...
		if (ownerId) {
			...
			if (!harmfulField && targetPlayer || (OTSYS_TIME() - createTime <= 5000) || creature->hasBeenAttacked(ownerId)) {
