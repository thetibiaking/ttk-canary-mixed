...
void Spell::applyCooldownConditions(Player* player) const
{
	...
	if (cooldown > 0) {
		...
		if (spellCooldown > 0) {
			Condition* condition = Condition::createCondition(CONDITIONID_DEFAULT, CONDITION_SPELLCOOLDOWN, spellCooldown / g_config.getFloat(ConfigManager::RATE_SPELL_COOLDOWN), 0, false, spellId);
	...
	if (groupCooldown > 0) {
		...
		if (spellGroupCooldown > 0) {
			Condition* condition = Condition::createCondition(CONDITIONID_DEFAULT, CONDITION_SPELLGROUPCOOLDOWN, spellGroupCooldown / g_config.getFloat(ConfigManager::RATE_SPELL_COOLDOWN), 0, false, group);
			player->addCondition(condition);
		}
	}
	...
	if (secondaryGroupCooldown > 0) {
		...
		if (spellSecondaryGroupCooldown > 0) {
			Condition* condition = Condition::createCondition(CONDITIONID_DEFAULT, CONDITION_SPELLGROUPCOOLDOWN, spellSecondaryGroupCooldown / g_config.getFloat(ConfigManager::RATE_SPELL_COOLDOWN), 0, false, secondaryGroup);
			player->addCondition(condition);
		}
	}
}
...
bool RuneSpell::executeUse(Player* player, Item* item, const Position&, Thing* target, const Position& toPosition, bool isHotkey)
{
	...
	} else if (getPzOnUse() && target && target->getCreature()) {
		if (Player* targetPlayer = target->getCreature()->getPlayer()) {
