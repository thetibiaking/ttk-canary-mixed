...
Condition* Condition::createCondition(ConditionId_t id, ConditionType_t type, int32_t ticks, int32_t param/* = 0*/, bool buff/* = false*/, uint32_t subId/* = 0*/)
{
    ...
    case CONDITION_ROOTED:
		...
		case CONDITION_SPECIALPOTION_COOLDOWN:
		case CONDITION_SPECIALPOTIONEFFECT_COOLDOWN:
		case CONDITION_SPECIALPOTION_EFFECT:
...
bool ConditionAttributes::setParam(ConditionParam_t param, int64_t value)
{
  ...
  case CONDITION_PARAM_BUFF_DAMAGERECEIVED: {
	...
	case CONDITION_PARAM_BUFF_INFLUENCED_DAMAGEDEALT: {
		buffsPercent[BUFF_INFLUENCED_DAMAGEDEALT] = std::max<int32_t>(0, value);
		return true;
	}

	case CONDITION_PARAM_BUFF_INFLUENCED_DAMAGERECEVIED: {
		buffsPercent[BUFF_INFLUENCED_DAMAGERECEVIED] = std::max<int32_t>(0, value);
		return true;
	}
...
bool ConditionDamage::doDamage(Creature* creature, int64_t healthChange)
{
  ...
  damage.canTrigerCharm = false;
  damage.canTrigerLeach = false;
