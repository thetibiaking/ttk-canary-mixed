...
int32_t Monster::getReflectPercent(CombatType_t reflectType) const {
...
void Monster::updateIdleStatus()
{
	...
	if (conditions.empty()) {
		if (!isFiendish() && !isSummon() && targetList.empty()) {
			idle = true;
		}
...
void Monster::onThink(uint32_t interval)
{
	...
	if (isFiendish()) {
		if (fiendRemoveTime <= time(nullptr)) {
...
void Monster::dropLoot(Container* corpse, Creature*)
{
	if (corpse && lootDrop) {
		...
		if (killer && (isInfluenced() || isFiendish())) {
			uint16_t dustStacks = uniform_random(influenceStacks, (influenceStacks * 3));
			Party* party = killer->getParty();
			if (party) {
				party->gainExaltedDust(dustStacks);
			} else {
				std::ostringstream text;
				if (killer->getDust() < killer->getMaxDust()) {
					uint16_t dustReward = dustStacks + killer->getDust() > killer->getMaxDust() ? (killer->getMaxDust() - killer->getDust()) : dustStacks;
					killer->setDust(dustReward);

					text << "You received " << dustReward << " dust for the Exaltation Forge. ";
					text << "You now have " << static_cast<uint16_t>(killer->getDust()) << " out of a maximum of " << static_cast<uint16_t>(killer->getMaxDust()) << " dust.";
				} else {
					text << "You did not receive " << dustStacks;
					text << " dust for the Exaltation Forge because you have already reached the maximum of " << static_cast<uint16_t>(killer->getMaxDust()) << " dust.";
				}

				TextMessage message(MESSAGE_LOOT, text.str());
				killer->sendTextMessage(message);
			}
		}
...
bool Monster::becomeStronger(bool fiendishMonster /* = false*/)
{
	if (isRemoved() || health != healthMax) {
		return false;
	}

	std::map<uint16_t, std::string> bestiaryMonsters = g_game.getBestiaryList();
	auto bestiary = bestiaryMonsters.find(getRaceId());
	if (bestiary == bestiaryMonsters.end()) {
		return false;
	}
	
	int32_t stacks = uniform_random(1, 5);
	if (fiendishMonster) {
		stacks = 15;
		fiendish = true;
		isIdle = false;
		fiendRemoveTime = time(nullptr) + (1 * 60 * 60);
		g_game.addCreatureCheck(this);
	} else {
		influenced = true;
	}

	influenceStacks = static_cast<uint16_t>(stacks);

	healthMax += (stacks * g_forge.getMonsterConfig().healthPercentStack) * healthMax / 100;
	health = std::max<int32_t>(health, healthMax);
	g_game.addCreatureHealth(this);

	setBuff(BUFF_INFLUENCED_DAMAGEDEALT, stacks * g_forge.getMonsterConfig().damagePerStack);
	setBuff(BUFF_INFLUENCED_DAMAGERECEVIED, -stacks * g_forge.getMonsterConfig().armorPerStack);
