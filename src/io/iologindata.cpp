...
bool IOLoginData::loadPlayer(Player* player, DBResult_ptr result)
{
  ...
  player->currentOutfit = player->defaultOutfit;
	player->dust = result->getNumber<uint16_t>("exalted_dust");
	uint8_t maxDust = result->getNumber<uint16_t>("max_exalted_dust");
	player->maxDust = (maxDust == 0 ? 100 : maxDust);
...
//load autoloot list set
  ...
  if ((result = db.storeQuery(query.str()))) {
    ...
    uint16_t itemId, bpId;
    while (propStreamList.read<uint16_t>(itemId) && propStreamList.read<uint16_t>(bpId)) {
    ...
    if (result = db.storeQuery(query.str())) {
      do {
        ...
        slot->bonusTimeLeft = result->getNumber<uint16_t>("bonus_time");
				...
				uint64_t freeRerollTime = result->getNumber<int64_t>("free_reroll");
				slot->freeRerollTimeStamp = (freeRerollTime > 0 ? freeRerollTime : OTSYS_TIME() + 1000);
  ...
  if (g_config.getBoolean(ConfigManager::TASK_HUNTING_ENABLED)) {
	...
	// Exalted Forge History
	loadExaltedForgeHistory(player);

	player->sendResourceBalance(RESOURCE_DUST, player->getSliver());
...
bool IOLoginData::savePlayer(Player* player)
{
  ...
  query << "`stamina` = " << player->getStaminaMinutes() << ',';
	...
	query << "`exalted_dust` = " << static_cast<uint16_t>(player->getDust()) << ',';
	query << "`max_exalted_dust` = " << static_cast<uint16_t>(player->getMaxDust()) << ',';
  ...
  if (!rewardList.empty()) {
    ...
    for (const auto& rewardId : rewardList) {
      ...
      if (!reward->empty() && (time(nullptr) - rewardId <= 60 * 60 * 24 * 4)) {
...
//Autoloot (save autoloot list)
  ...
  for (const auto& it : player->autoLootMap) {
    propWriteStreamAutoLoot.write<uint16_t>(it.first);
    propWriteStreamAutoLoot.write<uint16_t>(it.second);
  }
    ...
    if (item) {
      if (!item->unserializeAttr(propStream)) {
        SPDLOG_WARN("[IOLoginData::loadItems] - Failed to serialize");
			...
			if (item->getAttack() != 0 || item->getArmor() != 0 || item->getDefense() != 0) {
				checkItemIntegrity(item, player);
			}
...
void IOLoginData::checkItemIntegrity(Item* item, Player* player)
{
	if (!item || !player) {
		return;
	}
	...
	// Check attack
	if (item->getAttack() != it.attack) {
		std::cout << "Jogador: " << player->getName() << " possuia o item [" << item->getName() << "] com ATK : " << item->getAttack() << std::endl;
		item->setIntAttr(ITEM_ATTRIBUTE_ATTACK, it.attack);
	}

	// Check defense
	if (item->getDefense() != it.defense) {
		std::cout << "Jogador: " << player->getName() << " possuia o item [" << item->getName() << "] com DEF : " << item->getDefense() << std::endl;
		item->setIntAttr(ITEM_ATTRIBUTE_DEFENSE, it.defense);
	}

	// Check armor
	if (item->getArmor() != it.armor) {
		std::cout << "Jogador: " << player->getName() << " possuia o item [" << item->getName() << "] com ARM : " << item->getArmor() << std::endl;
		item->setIntAttr(ITEM_ATTRIBUTE_ARMOR, it.armor);
	}
}

bool IOLoginData::loadExaltedForgeHistory(Player* player)
{
	if (!player) {
		return false;
	}

	std::vector<ExaltedHistory> history;
	std::ostringstream query;
	query << "SELECT `timestamp`, `action`, `message`, `error` FROM `forge_history` WHERE `player_id` = " << player->getGUID();
	DBResult_ptr result = Database::getInstance().storeQuery(query.str());
	if (result) {
		do {
			ExaltedHistory info;
			info.time = result->getNumber<uint32_t>("timestamp");
			info.action = static_cast<ForgeAction_t>(result->getNumber<uint16_t>("action"));
			info.message = result->getString("message");
			info.error = static_cast<uint8_t>(result->getNumber<uint16_t>("error"));
			history.emplace_back(info);
			history.shrink_to_fit();
		} while (result->next());
	}

	player->forgeHistory = history;
	return true;
}
