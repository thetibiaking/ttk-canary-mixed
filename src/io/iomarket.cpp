...
MarketOfferList IOMarket::getActiveOffers(MarketAction_t action, uint16_t itemId, uint16_t tier)
{
	...
	query << "SELECT `id`, `amount`, `price`, `created`, `anonymous`, (SELECT `name` FROM `players` WHERE `id` = `player_id`) AS `player_name` FROM `market_offers` WHERE `sale` = " << action << " AND `itemtype` = " << itemId << " AND `tier` = " << tier;
...
MarketOfferList IOMarket::getOwnOffers(MarketAction_t action, uint32_t playerId)
{
	...
	std::ostringstream query;
	query << "SELECT `id`, `tier`, `amount`, `price`, `created`, `itemtype` FROM `market_offers` WHERE `player_id` = " << playerId << " AND `sale` = " << action;
...
HistoryMarketOfferList IOMarket::getOwnHistory(MarketAction_t action, uint32_t playerId)
{
	...
	std::ostringstream query;
	query << "SELECT `itemtype`, `tier`, `amount`, `price`, `expires_at`, `state` FROM `market_history` WHERE `player_id` = " << playerId << " AND `sale` = " << action;
...
MarketOfferEx IOMarket::getOfferByCounter(uint32_t timestamp, uint16_t counter)
{
	...
	std::ostringstream query;
	query << "SELECT `id`,  `tier`, `sale`, `itemtype`, `amount`, `created`, `price`, `player_id`, `anonymous`, (SELECT `name` FROM `players` WHERE `id` = `player_id`) AS `player_name` FROM `market_offers` WHERE `created` = " << created << " AND (`id` & 65535) = " << counter << " LIMIT 1";
...
void IOMarket::createOffer(uint32_t playerId, MarketAction_t action, uint32_t itemId, uint16_t tier, uint16_t amount, uint64_t price, bool anonymous)
{
	std::ostringstream query;
	query << "INSERT INTO `market_offers` (`tier`, `player_id`, `sale`, `itemtype`, `amount`, `price`, `created`, `anonymous`) VALUES (" << tier << ',' << playerId << ',' << action << ',' << itemId << ',' << amount << ',' << price << ',' << time(nullptr) << ',' << anonymous << ')';
...
void IOMarket::appendHistory(uint32_t playerId, MarketAction_t type, uint16_t itemId, uint16_t tier, uint16_t amount, uint64_t price, time_t timestamp, MarketOfferState_t state)
{
	std::ostringstream query;
	query << "INSERT INTO `market_history` (`player_id`, `sale`, `itemtype`, `tier`, `amount`, `price`, `expires_at`, `inserted`, `state`) VALUES ("
		<< playerId << ',' << type << ',' << itemId << ',' << tier << ',' << amount << ',' << price << ','
...
bool IOMarket::moveOfferToHistory(uint32_t offerId, MarketOfferState_t state)
{
	...
	std::ostringstream query;
	query << "SELECT `player_id`, `sale`, `itemtype`, `tier`, `amount`, `price`, `created` FROM `market_offers` WHERE `id` = " << offerId;
	...
	appendHistory(result->getNumber<uint32_t>("player_id"), static_cast<MarketAction_t>(result->getNumber<uint16_t>("sale")), result->getNumber<uint16_t>("itemtype"), result->getNumber<uint16_t>("tier"), result->getNumber<uint16_t>("amount"), result->getNumber<uint32_t>("price"), result->getNumber<uint32_t>("created") + marketOfferDuration, state);
	return true;
}
...
{
	std::ostringstream query;
	query << "SELECT `sale`, `itemtype`, `tier`, COUNT(`price`) AS `num`, `tier` AS `itemTier`, MIN(`price`) AS `min`, MAX(`price`) AS `max`, SUM(`price`) AS `sum` FROM `market_history` WHERE `state` = " << OFFERSTATE_ACCEPTED << " GROUP BY `itemtype`, `tier`, `sale`";
	DBResult_ptr result = Database::getInstance().storeQuery(query.str());
	if (!result) {
		return;
	}
	...
	do {
		...
		statistics.itemTier = result->getNumber<uint32_t>("itemTier");
		...
		if (result->getNumber<uint16_t>("sale") == MARKETACTION_BUY) {
			purchaseStatistics.emplace(itemId, statistics);
		} else {
			saleStatistics.emplace(itemId, statistics);
		}
	} while (result->next());
}
...
{
	for (auto it : purchaseStatistics) {
		if (it.first == itemId && it.second.itemTier == tier) {
			return new MarketStatistics(it.second);
		}
	}
	return nullptr;
}
