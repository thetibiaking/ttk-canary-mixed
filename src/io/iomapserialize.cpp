...
bool IOMapSerialize::loadContainer(PropStream& propStream, Container* container, Cylinder* parent)
{
	...
	if (iType.moveable || !tile || iType.isCarpet() || iType.isBed()) {
		...
		if (item) {
			if (item->unserializeAttr(propStream)) {
				...
				if (container && !loadContainer(propStream, container, parent)) {
					delete item;
					return false;
				}

				if (item->getAttack() != 0 || item->getArmor() != 0 || item->getDefense() != 0) {
					checkItemIntegrity(item, parent);
				}
		...
		if (item) {
			...
			if (dummy) {
				...
				if (container) {
					if (!loadContainer(propStream, container, parent)) {
						return false;
					}
...
void IOMapSerialize::checkItemIntegrity(Item* item, Cylinder* parent)
{
	if (!item || !parent) {
		return;
	}
	...
	if (item->getAttack() != it.attack) {
		std::cout << "Position: (" << pos.x << " " << pos.y << " " << static_cast<uint16_t>(pos.z) << ") possuia o item [" << item->getName() << "] com ATK: " << item->getAttack() << std::endl;
		item->setIntAttr(ITEM_ATTRIBUTE_ATTACK, it.attack);
	}

	// Check defense
	if (item->getDefense() != it.defense) {
		std::cout << "Position: (" << pos.x << " " << pos.y << " " << static_cast<uint16_t>(pos.z) << ") possuia o item [" << item->getName() << "] com DEF: " << item->getDefense() << std::endl;
		item->setIntAttr(ITEM_ATTRIBUTE_DEFENSE, it.defense);
	}

	// Check armor
	if (item->getArmor() != it.armor) {
		std::cout << "Position: (" << pos.x << " " << pos.y << " " << static_cast<uint16_t>(pos.z) << ") possuia o item [" << item->getName() << "] com ARM: " << item->getArmor() << std::endl;
