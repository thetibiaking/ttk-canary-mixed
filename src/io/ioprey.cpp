...
PreySlot::PreySlot(PreySlot_t id) :
		...
		reloadBonusValue();
		reloadBonusType();
		freeRerollTimeStamp = OTSYS_TIME() + g_config.getNumber(ConfigManager::PREY_FREE_REROLL_TIME) * 1000;
}
...
{
	auto minBonusPercent = static_cast<uint16_t>(g_config.getNumber(ConfigManager::PREY_BONUS_PERCENT_MIN));
	auto maxBonusPercent = static_cast<uint16_t>(g_config.getNumber(ConfigManager::PREY_BONUS_PERCENT_MAX));
...
TaskHuntingSlot::TaskHuntingSlot(PreySlot_t id) :
									id(id) {
	freeRerollTimeStamp = OTSYS_TIME() + g_config.getNumber(ConfigManager::TASK_HUNTING_FREE_REROLL_TIME) * 1000;
}
		...
		if (PreySlot* slot = player->getPreySlotById(static_cast<PreySlot_t>(slotId));
			...
			if (slot->bonusTimeLeft <= amount) {
				if (slot->option == PreyOption_AutomaticReroll) {
					if (player->usePreyCards(static_cast<uint16_t>(g_config.getNumber(ConfigManager::PREY_BONUS_REROLL_PRICE)))) {
						slot->reloadBonusValue();
						slot->reloadBonusType();
						slot->bonusTimeLeft = static_cast<uint16_t>(g_config.getNumber(ConfigManager::PREY_BONUS_TIME));
					...
					if (player->usePreyCards(static_cast<uint16_t>(g_config.getNumber(ConfigManager::PREY_SELECTION_LIST_PRICE)))) {
						slot->bonusTimeLeft = static_cast<uint16_t>(g_config.getNumber(ConfigManager::PREY_BONUS_TIME));
							...
							uint16_t raceId) const
{
	...
	if (action == PreyAction_ListReroll) {
		if (slot->freeRerollTimeStamp > OTSYS_TIME() && !g_game.removeMoney(player, player->getPreyRerollPrice(), 0, true)) {
			...
			slot->freeRerollTimeStamp = OTSYS_TIME() + g_config.getNumber(ConfigManager::PREY_FREE_REROLL_TIME) * 1000;
		}
		...
		if (!slot->isOccupied()) {
			...
			return;
		} else if (!player->usePreyCards(static_cast<uint16_t>(g_config.getNumber(ConfigManager::PREY_BONUS_REROLL_PRICE)))) {
		...
		if (option == PreyOption_AutomaticReroll && player->getPreyCards() < static_cast<uint64_t>(g_config.getNumber(ConfigManager::PREY_BONUS_REROLL_PRICE))) {
			player->sendMessageDialog("You don't have enought prey cards to enable automatic reroll when your slot expire.");
			return;
		} else if (option == PreyOption_Locked && player->getPreyCards() < static_cast<uint64_t>(g_config.getNumber(ConfigManager::PREY_SELECTION_LIST_PRICE))) {
									...
									uint16_t raceId) const
{
	...
	if (action == PreyTaskAction_ListReroll) {
		if (slot->disabledUntilTimeStamp >= OTSYS_TIME()) {
			...
			slot->freeRerollTimeStamp = OTSYS_TIME() + g_config.getNumber(ConfigManager::TASK_HUNTING_FREE_REROLL_TIME) * 1000;
		}
		...
		if (!player->usePreyCards(static_cast<uint16_t>(g_config.getNumber(ConfigManager::TASK_HUNTING_BONUS_REROLL_PRICE)))) {
		...
		if (slot->disabledUntilTimeStamp >= OTSYS_TIME()) {
			...
			player->sendMessageDialog(ss.str());
			return;
		} else if (!player->usePreyCards(static_cast<uint16_t>(g_config.getNumber(ConfigManager::TASK_HUNTING_SELECTION_LIST_PRICE)))) {
		...
		if (const TaskHuntingOption* option = GetTaskRewardOption(slot)) {
			...
			player->sendMessageDialog(ss.str());
			slot->reloadMonsterGrid(player->getTaskHuntingBlackList(), player->getLevel());
			slot->disabledUntilTimeStamp = OTSYS_TIME() + g_config.getNumber(ConfigManager::TASK_HUNTING_LIMIT_EXHAUST) * 1000;
		}
	} else {
