...
class IOMarket
{
	public:
		...
		static void createOffer(uint32_t playerId, MarketAction_t action, uint32_t itemId, uint16_t tier, uint16_t amount, uint64_t price, bool anonymous);
		static void acceptOffer(uint32_t offerId, uint16_t amount);
		static void deleteOffer(uint32_t offerId);

		static void appendHistory(uint32_t playerId, MarketAction_t type, uint16_t itemId, uint16_t tier, uint16_t amount, uint64_t price, time_t timestamp, MarketOfferState_t state);
		...
		std::multimap<uint16_t, MarketStatistics>& getPurchaseStatistics() {
			return purchaseStatistics;
		}
		std::multimap<uint16_t, MarketStatistics>& getSaleStatistics() {
	...
	private:
		...
		std::multimap<uint16_t, MarketStatistics> purchaseStatistics;
		std::multimap<uint16_t, MarketStatistics> saleStatistics;
};
