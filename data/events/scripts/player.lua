...
function Player:onLookInBattleList(creature, distance)
	local description = "You see " .. creature:getDescription(distance)
	if creature:isMonster() then
		local masterupdate = creature:getMaster()
		local summonsupdate = {'avatar of storm','avatar of steel','avatar of nature','avatar of light'}
		if masterupdate and table.contains(summonsupdate, creature:getName():lower()) then
			description = description..' (Master: ' .. masterupdate:getName() .. '). \z
				It will disappear in ' .. getTimeinWords(masterupdate:getStorageValue(Storage.PetSummonUpdate) - os.time())
		end
	end
	if self:getGroup():getAccess() then
		local str = "%s\nHealth: %d / %d"
			...
			if zone == ZONE_PROTECTION then
				if self:getStamina() < 2520 then
					if not event then
						...
						self:sendTextMessage(MESSAGE_STATUS, "[STAMINA] Regenerate...")
						staminaBonus.eventsPz[self:getId()] = addEvent(addStamina, delay * 60 * 1000, nil, self:getId(), delay * 60 * 1000)
					end
				end
			else
				if event then
					self:sendTextMessage(MESSAGE_STATUS, "[STAMINA] Saiu do PZ.")
					stopEvent(event)
