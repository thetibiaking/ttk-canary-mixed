...
local config = {
	...
	teleportPosition = Position(33456, 31430, 13),
	...
	if item.itemid == 9825 then		
		...
		for i = 1, #config.playerPositions do
			...
			if participant and participant:isPlayer() then
				...
				if config.daily and participant:getStorageValue(Storage.Boos.CountVlarkorth) > os.time() then
		...
		for i = 1, #team do
			...
			-- Assign boss timer
			team[i]:setStorageValue(Storage.Boos.CountVlarkorth, os.time() + 20*60*60) -- 20 hours
		end
