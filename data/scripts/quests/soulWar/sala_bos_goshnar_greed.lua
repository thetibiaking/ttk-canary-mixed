...
local config = {				
	...
	requiredLevel = 400,
	daily = true,
	roomCenterPosition = Position(34060, 31092, 11),
	playerPositions = {
	...
	teleportPosition = Position(34064, 31098, 11),
	...
	if item.itemid == 9825 then		
		...
		for i = 1, #config.playerPositions do
			...
			if participant and participant:isPlayer() then
				...
				if config.daily and participant:getStorageValue(Storage.Boos.goshnarGreed) > os.time() then
		...
		-- Spawn boss
		Game.createMonster("Goshnar's Greed", config.bossPosition)
		...
		for i = 1, #team do
			...
			-- Assign boss timer
			team[i]:setStorageValue(Storage.Boos.goshnarGreed, os.time() + 20*60*60) -- 20 hours
		end
