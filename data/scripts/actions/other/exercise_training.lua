...
local houseDummies = {32145, 32146, 32147, 32148}
local magoDummies = {32143, 32144}
                        ...
                        if exercise:hasAttribute(ITEM_ATTRIBUTE_CHARGES) then
                            ...
                            if charges_n >= 1 then
                                ...
                                if exercise:getAttribute(ITEM_ATTRIBUTE_CHARGES) == 0 then
                                    ...
                                    local training = addEvent(startTraining, delay, playerId,startPosition,itemid,tilePosition,bonusDummy,dummyId)
...
function exerciseTraining.onUse(player, item, fromPosition, target, toPosition, isHotkey)
    ...
    if target:isItem() then
        ...
        elseif isInArray(magoDummies,target:getId()) then
                if not skills[item.itemid].range and (startPos:getDistance(target:getPosition()) > 1) then
                    player:sendTextMessage(MESSAGE_FAILURE, "Get closer to the dummy.")
                    stopEvent(training)
                    return true
				...
				if not player:isVip() then
                    ...
                    player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "You don't have a Vip")
                    return true
				end
                player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "You started training.")
                startTraining(player:getId(),startPos,item.itemid,target:getPosition(), true, target:getId())
