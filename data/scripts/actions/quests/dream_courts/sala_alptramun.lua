	...
	if item.itemid == 9825 then		
		...
		for i = 1, #config.playerPositions do
			...
			if participant and participant:isPlayer() then
				...
				if config.daily and participant:getStorageValue(Storage.Boos.Alptramun) > os.time() then
		...
		-- Spawn boss
		Game.createMonster("Alptramun", config.bossPosition)
		...
		for i = 1, #team do
			...
			-- Assign boss timer
			team[i]:setStorageValue(Storage.Boos.Alptramun, os.time() + 20*60*60) -- 20 hours
		end
