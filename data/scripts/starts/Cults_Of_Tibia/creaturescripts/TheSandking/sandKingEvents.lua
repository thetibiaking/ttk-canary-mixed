local function spawnSandBoss(name, frompos, topos)
	...
	for _x= frompos.x, topos.x, 1 do
		for _y= frompos.y, topos.y, 1 do
			for _z= frompos.z, topos.z, 1 do
				...
				if tile and tile:getTopCreature() and tile:getTopCreature():isMonster() and tile:getTopCreature():getName() == "Sand Brood" then
					spawn = false
					break
				end
			end
	...
	for _x= frompos.x, topos.x, 1 do
		for _y= frompos.y, topos.y, 1 do
			for _z= frompos.z, topos.z, 1 do
				local tile = Tile(Position(_x, _y, _z))
				if tile and tile:getTopCreature() and tile:getTopCreature():getName() == "Sand Vortex" then
	...
	local str = Game.getStorageValue(GlobalStorage.CultsOfTibia.SandKingThink)
	local boss = Game.createMonster("the sandking fake", Position( 33099, 31859, 15))
	if str < 4 then
		boss:registerEvent("SandkingThink")
	end
	Game.setStorageValue(GlobalStorage.CultsOfTibia.SandKingThink, str + 1)
...
function sandkingThink.onThink(creature)
	...
	if creature:getName() ~= "The Sandking" then
		return true
	end
...
function sandkingDeath.onDeath(creature, attacker, corpse)
	...
	if creature:getName() == "Sand Brood" then
		addEvent(
	...
	local stg = Game.getStorageValue(GlobalStorage.CultsOfTibia.SandKingThink)
	if stg == 4 then
		local monster = Game.createMonster("the sandking", Position( 33099, 31859, 15))
		monster:setHealth(monster:getMaxHealth()/2)
		addEvent(function()
			Game.setStorageValue(GlobalStorage.CultsOfTibia.SandKingThink, 5)
		end, 2000)
	end
	return true
end
...
function sandHealth.onHealthChange(creature, attacker, primaryDamage, primaryType, secondaryDamage, secondaryType, origin)
	...
	if creature:getName() == "Sand Vortex" then
		...
		if attacker and attacker:isPlayer() and not attacker:isRemoved() then
