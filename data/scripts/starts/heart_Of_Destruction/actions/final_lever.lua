				...
				if #storeHunger < 1 or #storeDestruction < 1 or #storeRage < 1 then
				 ...
				 -- Check participant boss timer
                    for i = 1, #storeDestruction do
						...
						if teamDestruction:getStorageValue(14333) > os.time() then
							notAllowed = true
                            teamDestruction:getPosition():sendMagicEffect(CONST_ME_POFF)
                        end
					...
					for i = 1, #storeHunger do
						...
						if teamHunger:getStorageValue(14333) > os.time() then
							notAllowed = true
							teamHunger:getPosition():sendMagicEffect(CONST_ME_POFF)
						end
					end
					
					for i = 1, #storeRage do
						teamRage = storeRage[i]
						if teamRage:getStorageValue(14333) > os.time() then
							notAllowed = true
							teamRage:getPosition():sendMagicEffect(CONST_ME_POFF)
--							players:sendCancelMessage("Not all players are ready yet from last battle.")
--							return true
						end
					end
					
					if notAllowed == true then
						mainPlayer = storeDestruction[1]
						mainPlayer:sendCancelMessage("Not all players are ready yet from last battle.")
						return true
					end					
