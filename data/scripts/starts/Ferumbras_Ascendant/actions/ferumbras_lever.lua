local config = {
	bossName = "Ferumbras Soul Splinter",
	...
	if item.itemid == 9825 then
				...
				local playerTile = Tile(Position(x, y, 14)):getTopCreature()
		     	if playerTile and playerTile:isPlayer() then
					if playerTile:getStorageValue(Storage.FerumbrasAscension.FerumbrasTimer) > os.time() then
						player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "You or a member in your team have to wait 14 days to face Ferumbras again!")
						item:transform(9826)
					    return true
					end
				end
		...
		for i = 1, #spectators do
			...
			if spectator:isMonster() then
				...
				Game.createMonster(config.bossName, Position(33383 ,31463,14), true, true) 
				Game.createMonster(config.bossName, Position(33383, 31470, 14), true, true)
				Game.createMonster(config.bossName, Position(33383, 31479, 14), true, true)
				Game.createMonster(config.bossName, Position(33392, 31467, 14), true, true)
				Game.createMonster(config.bossName, Position(33399, 31467, 14), true, true)
				Game.createMonster(config.bossName, Position(33401, 31480, 14), true, true)
				Game.createMonster(config.bossName, Position(33383, 31483, 14), true, true)
		...
		for x = 33269, 33271 do
			for y = 31477, 31481 do
				...
				--	end
					...
					local vortex1 = Game.createItem(22455, 1, Position(33384, 31467, 14))
					local vortex2 = Game.createItem(22455, 1, Position(33384, 31472, 14))
					local vortex3 = Game.createItem(22455, 1, Position(33400, 31470, 14))
					local vortex4 = Game.createItem(22455, 1, Position(33400, 31475, 14))
					local vortex5 = Game.createItem(22455, 1, Position(33392, 31481, 14))
					item:transform(9826)
				end
			end
		end
