...
local function FixBugError(cid)
local player = Player(cid)
if not player then return true end
	local playerBox = player:getInbox()
	local slotbugado = player:getSlotItem(CONST_SLOT_STORE_INBOX)
	if not slotbugado then return true end
	player:sendTextMessage(MESSAGE_STATUS_WARNING, "Mensagem de seguran�a, se voc� tinha intens dentro do store inbox, foi pro seu inbox no depot, att Equipe.")
	local lastIndex = slotbugado:getSize() - 1
	for i=0,lastIndex do 
		local item = slotbugado:getItem(i)
		if item then
			item:moveTo(playerBox)
		end
	end
end
...
function playerLogin.onLogin(player)
	...
	if player:getStorageValue(9876544) == -1 then
		FixBugError(player:getId())
		player:setStorageValue(9876544, 1)
	end
