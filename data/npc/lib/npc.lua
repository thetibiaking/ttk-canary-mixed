...
function doNpcSellItem(cid, itemId, amount, subType, ignoreCap, inBackpacks, backpack)
    ...
    if ItemType(itemId):isStackable() then
        ...
        local amountIndex = math.max(1, math.ceil(amount / 100))
        local stuff
        if inBackpacks then
            stuff = Game.createItem(backpack, 1)
            for i = 1, amountIndex do
                local internalAmount = remainingAmount > 100 and 100 or remainingAmount
                if stuff:addItem(itemId, internalAmount) == nil then
                    return amount - remainingAmount, 0
                else
                    remainingAmount = remainingAmount - internalAmount
                end
            end
            ...
            for i = 1, amountIndex do
                local internalAmount = remainingAmount > 100 and 100 or remainingAmount
                if player:addItem(itemId, internalAmount) == nil then
                    return amount - remainingAmount, 0
                else
                    remainingAmount = remainingAmount - internalAmount
                end
