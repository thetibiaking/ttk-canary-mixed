...
local chargeItem = {
	...
	['ring of souls'] = {noChargeID = 37471, ChargeID = 37456},
	['turtle amulet'] = {noChargeID = 44070, ChargeID = 44068},
	['alicorn ring'] = {noChargeID = 44017, ChargeID = 44015},
	['arboreal ring'] = {noChargeID = 44023, ChargeID = 44021},
	['arcanomancer sigil'] = {noChargeID = 44020, ChargeID = 44018},
	['spiritthorn ring'] = {noChargeID = 44014, ChargeID = 44012}
}
...
function creatureSayCallback(cid, type, msg)
	...
	elseif msgcontains(msg, 'enchant') then
		npcHandler:say("The following items can be enchanted: {pendulet}, {sleep shawl}, {blister ring}, {alicorn ring}, {arboreal ring}, {arcanomancer sigil}, {spiritthorn ring}, {theurgic amulet}, {turtle amulet}, {ring of souls}. Make you choice!", cid)
		npcHandler.topic[cid] = 1
	elseif isInArray({'pendulet', 'sleep shawl', 'blister ring', 'theurgic amulet',}, msg:lower()) and npcHandler.topic[cid] == 1 then
		npcHandler:say("Should I enchant all the items per 2 ".. ItemType(Npc():getCurrency()):getPluralName():lower() .."?", cid)
		charge = msg:lower()
		npcHandler.topic[cid] = 2
	elseif isInArray({'turtle amulet', 'alicorn ring', 'arboreal ring', 'arcanomancer sigil','spiritthorn ring', 'ring of souls'}, msg:lower()) and npcHandler.topic[cid] == 1 then
		npcHandler:say("Should I enchant all the items per 5 ".. ItemType(Npc():getCurrency()):getPluralName():lower() .."?", cid)
		charge = msg:lower()
	...
	elseif npcHandler.topic[cid] == 5 then
		if msgcontains(msg, 'yes') then
			if not chargeItem[charge] then
				...
				if (player:getItemCount(Npc():getCurrency()) >= 5) and (player:getItemCount(chargeItem[charge].noChargeID) >= 1) then
					player:removeItem(Npc():getCurrency(), 5)
					player:removeItem(chargeItem[charge].noChargeID, 1)
					local itemAdd = player:addItem(chargeItem[charge].ChargeID, 1)
					npcHandler:say("Ah, excellent. Here is your " .. itemAdd:getName():lower() .. ".", cid)
				else
