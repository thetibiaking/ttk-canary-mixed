...
function Player:getTempStorageValue(key)
	if not tempStorage[self:getId()] then
		tempStorage[self:getId()] = { }
	end
	return tempStorage[self:getId()][key]
end

function Player:setTempStorageValue(key, value)
	if not tempStorage[self:getId()] then
		tempStorage[self:getId()] = { }
	end
	tempStorage[self:getId()][key] = value
end
