if not(WheelOfDestinySystem) then
    WheelOfDestinySystem = {
        config = {
            ...
            pointsPerLevel = 1,
            onlyPremiumPlayersCanUse = true, -- This value seems to be hardcoded on the client, only can be changed when editing the client (Default TRUE)
            onlyPromotedPlayersCanUse = true, -- This value seems to be hardcoded on the client, only can be changed when editing the client (Default TRUE)
...
dofile('data/lib/core/wheelofdestiny/protocol.lua')
dofile('data/lib/core/wheelofdestiny/knight.lua')
dofile('data/lib/core/wheelofdestiny/druid.lua')
dofile('data/lib/core/wheelofdestiny/paladin.lua')
dofile('data/lib/core/wheelofdestiny/sorcerer.lua')
...
WheelOfDestinySystem.initializeGlobalData = function(reload)
    if not(WheelOfDestinySystem.config.enabled) then
        if not(reload) then
            print("Loading wheel of destiny...", "[Disabled by admin]")
        end
        return false
    end
    ...
    if not(reload) then
        print("Loading wheel of destiny...", "[Success]")
    else
        print("Reloading wheel of destiny...", "[Success]")
    end
    return true
end
    ...
    for _, spellTable in ipairs(spellsTable) do
        if (spellTable.name == spell and spellTable.grade[stage] ~= nil) then
            if (spellTable.grade[stage].increase ~= nil) then
                if (spellTable.grade[stage].increase.area ~= nil) then
                    return spellTable.grade[stage].increase.area
                end
            end
        end
    end
