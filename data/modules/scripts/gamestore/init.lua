...
GameStore.OfferTypes = {
	...
	OFFER_TYPE_ITEM_ICON = 26,
	OFFER_TYPE_HOUSE_BED = 27
}
...
function convertType(type)
	local types = {
		...
		[GameStore.OfferTypes.OFFER_TYPE_HOUSE_BED] = GameStore.ConverType.SHOW_ITEM,
		...
		[GameStore.OfferTypes.OFFER_TYPE_ITEM_ICON] = GameStore.ConverType.SHOW_NONE,
	}
...
function parseTransferCoins(playerId, msg)
	...
	GameStore.insertHistory(accountId, GameStore.HistoryTypes.HISTORY_TYPE_NONE, player:getName() .. " transfered you this amount.", 0, amount)
	GameStore.insertHistory(player:getAccountId(), GameStore.HistoryTypes.HISTORY_TYPE_NONE, "You transfered this amount to " .. reciver, 0, -1 * amount) -- negative
end
...
function parseBuyStoreOffer(playerId, msg)
	...
	if (table.contains(GameStore.OfferTypes, offer.type) == false)                      -- we've got an invalid offer type
		...
		or (offer.type ~= GameStore.OfferTypes.OFFER_TYPE_NAMECHANGE and
			...
			offer.type ~= GameStore.OfferTypes.OFFER_TYPE_HUNTINGSLOT and
	...
	if (offer.coinType == GameStore.CointType.Tournament and not player:canRemoveTournamentCoins(offerPrice)) or (offer.coinType ~= GameStore.CointType.Tournament and not player:canRemoveCoins(offerPrice)) then
	...
	local pcallOk, pcallError = pcall(function()
		...
		elseif offer.type == GameStore.OfferTypes.OFFER_TYPE_HOUSE_BED          then GameStore.processHouseBedPurchase(player, offer.itemtype, offer.count, offer.name)
		...
		elseif offer.type == GameStore.OfferTypes.OFFER_TYPE_ITEM_ICON            then GameStore.processHouseRelatedPurchase(player, offer.itemtype, offer.count)
		else
	...
	if configure ~= GameStore.ConfigureOffers.SHOW_CONFIGURE then
		if offer.coinType == GameStore.CointType.Tournament then
			player:removeTournamentCoinsBalance(offerPrice)
		else
			player:removeCoinsBalance(offerPrice)
		end
		GameStore.insertHistory(player:getAccountId(), GameStore.HistoryTypes.HISTORY_TYPE_NONE, offer.name, offer.coinType, (offerPrice) * -1)
		local message = string.format("You have purchased %s for %d %scoins.", offer.name, offerPrice, (offer.coinType == GameStore.CointType.Tournament and "tournament " or ""))
		...
		if (canBuy.disabled == 8) then
			for index, disableTable in ipairs(disableReasons) do
				if (canBuy.disabledReason == disableTable.reason) then
					offer.disabledReasonIndex = index
				end
			end

			if (offer.disabledReasonIndex == nil) then
				offer.disabledReasonIndex = #disableReasons
	...
	if count > 0 then
		for name, offer in pairs(offers) do
			...
			for _, off in ipairs(offer.offers) do
				...
				msg:addByte((off.disabledReasonIndex ~= nil) and 1 or 0)
				if (off.disabledReasonIndex ~= nil) then
					msg:addByte(0x01);
					msg:addU16(off.disabledReasonIndex)
				end
...
function sendStoreTransactionHistory(playerId, page, entriesPerPage)
	...
	for k, entry in ipairs(entries) do
		...
		msg:addByte(entry.coinType) -- 0 = transferable tibia coin, 1 = normal tibia coin, 2 = tournament coin
...
function sendUpdateCoinBalance(playerId)
	...
	msg:addU32(player:getCoinsBalance())
	msg:addU32(player:getCoinsBalance())
	msg:addU32(player:getTournamentCoinsBalance()) -- Tournament Coins
...
GameStore.insertHistory = function(accountId, mode, description, coinType, amount)
	return db.query(string.format("INSERT INTO `store_history`(`account_id`, `mode`, `description`, `coin_type`, `coin_amount`, `time`) VALUES (%s, %s, %s, %s, %s, %s)", accountId, mode, db.escapeString(description), coinType, amount, os.time()))
end
...
function GameStore.processHouseBedPurchase(player, offerId, offerCount, offerName)
	local iType = ItemType(offerId)
	local bedPart
	if iType then
		bedPart = iType:getBedPartOf()
	end
	if not bedPart or type(bedPart) ~= "number" or bedPart <= 0 then
		Spdlog.warn(string.format("[GameStore.processHouseBedPurchase] Failed to find bedpart of itemId: %s", offerId))
		return error({code = 0, message = "Please, try again later."})
	end
	local inbox = player:getSlotItem(CONST_SLOT_STORE_INBOX)
	if inbox and inbox:getEmptySlots() > 0 then
		local parcel = inbox:addItem(2595, 1)
		if parcel then
			local bedKit = parcel:addItem(26054, 1)
			if bedKit then
				bedKit:setAttribute(ITEM_ATTRIBUTE_DESCRIPTION, "You bought this item in the Store.\nUnwrap it in your own house to create a <" .. iType:getName() .. ">.")
				bedKit:setCustomAttribute("unWrapId", offerId)
			end
			local bedPartKit = parcel:addItem(26054, 1)
			if bedPartKit then
				bedPartKit:setAttribute(ITEM_ATTRIBUTE_DESCRIPTION, "You bought this item in the Store.\nUnwrap it in your own house to create a <" .. ItemType(bedPart):getName() .. ">.")
				bedPartKit:setCustomAttribute("unWrapId", bedPart)
			end
		end
		local fullWeight = parcel:getWeight()
		if(fullWeight > player:getFreeCapacity()) then
			return error({code = 0, message = "Please make sure you have free cap."})
		end
		parcel:setAttribute(ITEM_ATTRIBUTE_NAME, '' .. offerCount .. 'x ' .. offerName .. ' package.')
	else
		return error({code = 0, message = "Please make sure you have free slots in your store inbox."})
...
function GameStore.processExpBoostPuchase(player)
	...
	if currentExpBoostTime > 0 then
		return error({code = 1, message = "You already have Exp boost active."})
	end
	local expBoostCount = player:getStorageValue(GameStore.Storages.expBoostCount)
	if expBoostCount == -1 then
		expBoostCount = 0
	end
	if expBoostCount >= 5 then
		return error({code = 1, message = "You can only buy 5 exp boost per day."})
	end
...
function Player.setTournamentCoinsBalance(self, coins)
	db.query("UPDATE `accounts` SET `tournament_coins` = " .. coins .. " WHERE `id` = " .. self:getAccountId())
	return true
end

function Player.canRemoveTournamentCoins(self, coins)
	if self:getTournamentCoinsBalance() < coins then
		return false
	end
	return true
end

function Player.removeTournamentCoinsBalance(self, coins)
	if self:canRemoveTournamentCoins(coins) then
		return self:setTournamentCoinsBalance(self:getTournamentCoinsBalance() - coins)
	end

	return false
end

function Player.addTournamentCoinsBalance(self, coins, update)
	self:setTournamentCoinsBalance(self:getTournamentCoinsBalance() + coins)
...
function sendHomePage(playerId)
	...
	for p, offer in pairs(homeOffers)do
		...
		msg:addByte((offer.disabledReasonIndex ~= nil) and 1 or 0)
		if (offer.disabledReasonIndex ~= nil) then
			msg:addByte(0x01);
			msg:addU16(offer.disabledReasonIndex)
		end
...
function HandleHirelingNameChange(playerId, offer, newHirelingName)
	...
	local cb = function(playerId, data, hireling)
		...
		GameStore.insertHistory(player:getAccountId(), GameStore.HistoryTypes.HISTORY_TYPE_NONE, offer.name .. ' ('.. oldName .. '->' .. newHirelingName ..')', 0, (offer.price) * -1)
...
function HandleHirelingSexChange(playerId, offer)
	...
	local cb = function(playerId, data, hireling)
		...
		GameStore.insertHistory(player:getAccountId(), GameStore.HistoryTypes.HISTORY_TYPE_NONE, offer.name .. ' ('.. hireling:getName() ..')', 0, (offer.price) * -1)
